...
import com.hucheng.common.bean.HomeUser;
...
import com.hucheng.common.exception.SessionHomeTimeoutException;
 ...
 * @Package com.hucheng.chinawkb.interceptor
 * @Description: 系统前台用户登录拦截
 * @date 2017/11/28 19:27
 */
public class HomeInterceptor extends HandlerInterceptorAdapter {

    public final static Logger logger= LoggerFactory.getLogger(HomeInterceptor.class);
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        ...
        HomeUser user = (HomeUser) request.getSession().getAttribute(Constant.SESSION_ADMIN);
        if(user != null) {
            ...
            if (request.getHeader("x-requested-with") != null
                ...
                try{
                    out.print(new HttpResponse<>().homeTimeout("timeout"));
                    return false;
                }catch(Exception e){
                    throw new SessionHomeTimeoutException();//返回到配置文件中定义的路径
                }finally {
                    try{
                        out.close();
                    }catch (Exception e){}
